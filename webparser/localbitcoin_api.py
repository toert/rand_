...
class LocalBitcoin:
    ...
    def __init__(self, hmac_auth_key, hmac_auth_secret, debug = False):
        if hmac_auth_key is None:
            raise 'Got no hmac_auth_key'
        if hmac_auth_secret is None:
            raise 'Got no hmac_auth_secret'
    ...
    def sendRequest(self, endpoint, params, method):
        ...
        if method == 'get':
            response = requests.get(self.baseurl + endpoint, headers = headers, params = params, proxies=proxies)
        else:
            response = requests.post(self.baseurl + endpoint, headers = headers, data = params, proxies=proxies)
...
if __name__ == '__main__':
    ...
    client.sendRequest(endpoint='/api/ad/465921/',
                       params=params,
                       method='post')
